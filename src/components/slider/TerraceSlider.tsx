  ...
  const sortedTerraces = useMemo(() => {
    ...
    switch (orderBy) {
      ...
      case "nearby":
        ...
        console.log(
          "📍 Nearby terraces within 1km:",
          nearbyTerraces.map((t) => ({
            name: t.business_name,
            distance: t.distance?.toFixed(2),
          }))
        );

        return Array.isArray(nearbyTerraces) ? nearbyTerraces : [];

      case "default":
      default:
        return shuffleArray(terraces);
    }
  }, [list, orderBy, location]);
      ...
      <ScrollSnap>
        {Array.isArray(sortedTerraces) && sortedTerraces.length > 0 ? (
          ...
          <div className="p-4 siyaDark-text text-start border-siya-principal border-r-4 border-b-4 rounded-2xl bg-gray-100 text-balance">
            No s'han trobat terrasses sota aquests criteris, siusplau, prova amb
            altres filtres.
          </div>
        )}
      </ScrollSnap>
