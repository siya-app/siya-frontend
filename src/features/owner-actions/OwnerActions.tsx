...
function OwnerActions() {
  ...
  const [ownedTerrace, setOwnedTerrace] = useState<CustomTerraceType | null>(
    null
  );
  ...
  useEffect(() => {
    if (user && allUsers.length > 0 && allTerraces.length > 0) {
      ...
      if (currentUser) {
        const claimedTerrace = allTerraces.find(
          (t) => t.id === currentUser.id_terrace
        ) as CustomTerraceType | undefined;
  ...
  const handleUpdateTerrace = async (formData: TerraceUpdatePayload) => {
    try {
      ...
      const updateURL = `${import.meta.env.VITE_API_ALL_TERRACES}/${
        ownedTerrace?.id || ""
      }`;
    ...
    <>
      {user.role === "owner" && ownedTerrace && (
        ...
        cursor-pointer w-fit"
          ...
          <UpdateTerrace
            terrace={ownedTerrace}
            isOpen={isEditTerraceOpen}
            onClose={() => setIsEditTerraceOpen(false)}
            onSubmit={handleUpdateTerrace}
          />
          <TerraceUnclaim
